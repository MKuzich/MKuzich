<h1 align="center">Hi, I'm Mykhailo Kuzich 👋</h1>
<p align="center"><b>Full-Stack Developer</b> · React · TypeScript · Next.js · Node.js · Laravel</p>
<p align="center">📍 Emmen, Netherlands</p>

---

I build scalable, performant web apps end to end. Over 3+ years at **Webba** I've
shipped **24+ commercial websites and applications for 60+ clients** — usually
around **20% ahead of schedule** — and one of my Laravel platforms became my
team's internal reference architecture. I care about clean architecture, smooth
UX, and reliable APIs.

### 🧰 Tech Stack

![React](https://img.shields.io/badge/-React-20232A?style=flat-square&logo=react)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-000?style=flat-square&logo=next.js)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/-Express-000?style=flat-square&logo=express)
![Laravel](https://img.shields.io/badge/-Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white)
![Tailwind](https://img.shields.io/badge/-Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)

**Languages:** JavaScript · TypeScript · PHP  
**Frameworks:** React · Next.js · Node.js · Express · Laravel  
**Styling:** TailwindCSS · Material UI · Styled Components · Mantine  
**State & data:** Redux · React Query · Livewire  
**Databases:** PostgreSQL · MongoDB · MariaDB · SQL/NoSQL  
**Tools:** Git · GitHub · GitLab · GSAP · WordPress

### 📫 Get in touch

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mykhailo-kuzich/)
[![Email](https://img.shields.io/badge/-mykhailokuzich@gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:mykhailokuzich@gmail.com)

### 📊 GitHub Stats

<p>
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=MKuzich&show_icons=true&hide_border=true&count_private=true" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MKuzich&layout=compact&hide_border=true&langs_count=8" />
</p>
